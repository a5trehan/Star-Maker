...
SHAVES = 3 # Specify vector processors for device
...
with dai.Device(pipeline) as device:
    ...
    while(True):
        ...
        if (elapsed_time - last_launch_time) >= (launch_period - 3) and launch_count < num_launches:
            launch = True
        ...
        for t in trackletsData:
            # Dynamically set deviation tolerance and launch velocity
            deviation_tol = round((1/80)*t.spatialCoordinates.z + (35/4))
            max_speed = 4.1904*np.exp(t.spatialCoordinates.z*0.0008)
            speed = min(80, max_speed)

            if tracklets_length == 1:
                if launching:
                    if (elapsed_time - last_launch_time) < launch_period:
                        dc = max_speed * (elapsed_time - (last_launch_time + (launch_period - 3))) / 3
                        PWMA.ChangeDutyCycle(dc)
                        PWMB.ChangeDutyCycle(dc)
                else:
                    if t.status.name == "TRACKED" and not launch:
